...
describe("customer app", () => {
    ...
    describe("update should", () => {
        ...
        test.each<{ initialValue: boolean, newValue: boolean }>([
            ...
            const initialModel: Model = {
                customerId: 5,
                customer: { id: 5, name: "name", premiumSubscription: initialValue },
                customerEdition: { id: 5, name: "name", premiumSubscription: initialValue },
                error: "",
                loading: false,
            }
            ...
            expect(result.model).toEqual<Model>({
                ...initialModel,
                customerEdition: { id: 5, name: "name", premiumSubscription: newValue },
            })
            expect(result.effects).toEqual<Effect[]>([])
        })

        test("UpdatePremiumSubscription - do nothing when not in edit mode", () => {
             const initialModel: Model = {
                customerId: 5,
                customer: { id: 5, name: "name", premiumSubscription: true },
                customerEdition: null,
                error: "",
                loading: false,
            }
            ...
            const result = applyCommands([
                { kind: "UpdatePremiumSubscription", value: false }
            ], initialModel)
        ...
        test("CancelEdit", () => {
            const initialModel: Model = {
                customerId: 5,
                customer: { id: 5, name: "name", premiumSubscription: false },
                customerEdition: { id: 5, name: "new name", premiumSubscription: true },
                error: "",
                loading: false,
            }
        ...
        test("SaveCustomer", () => {
            ...
            expect(result.effects).toEqual<Effect[]>([
                { kind: "SaveCustomer", customer: { id: 5, name: "newName", premiumSubscription: true } }
            ])
        })

        test("SaveCustomer - do nothing when not in edit mode", () => {
            const initialModel: Model = {
                customerId: 5,
                customer: { id: 5, name: "oldName", premiumSubscription: false },
                customerEdition: null,
