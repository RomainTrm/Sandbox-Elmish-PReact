...
describe("customer app", () => {
    ...
    describe("update should", () => {
        ...
        test("EditCommand - Save", () => {
            ...
            expect(result.effects).toEqual<Effect[]>([
                { kind: "EditEffect", effect: { kind: "SaveCustomer", customer: { id: 5, name: "name", premiumSubscription: true } } }
            ])
        })
        ...
        test("EditCommand - NotifySaveSucceeded", () => {
            const initialModel: Model = {
                customerId: 5,
            ...
            const result = applyCommands([
                { kind: "EditCommand", subCommand: { kind: "NotifySaveSucceeded" } }
            ], initialModel)
        ...
        test("EditCommand - NotifySaveFailed", () => {
            const initialModel: Model = {
                customerId: 5,
                customer: { id: 5, name: "oldName", premiumSubscription: false },
                customerEdition: { id: 5, name: "newName", premiumSubscription: true },
            ...
            const result = applyCommands([
                { kind: "EditCommand", subCommand: { kind: "NotifySaveFailed", error: "error message" } }
            ], initialModel)
